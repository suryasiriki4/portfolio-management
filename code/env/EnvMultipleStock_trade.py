...
class StockEnvTrade(gym.Env):
    ...
    def modifyActions(self, actions):
        ...
        for stock in stocks:
            # print(i)
            path = "./env/twitter_prob_data/"+stock+"_twitter"+".csv"
            if os.path.isfile(path):
                recent_dates = []
                # print(path)
                df = pd.read_csv(path)
                tweets_prob_action_array = df.loc[df['date']==day]['action_prob'].values
                if (len(tweets_prob_action_array) == 0):
                    tweet_prob_action = 0
                else:
                    tweet_prob_action = tweets_prob_action_array[0]
                new_actions[i] = 0.7 * actions[i] + 0.3 * tweet_prob_action * HMAX_NORMALIZE
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            # print("before:")
            # print(actions)
            if self.model_name == "ensemble_pickle_nlp":
