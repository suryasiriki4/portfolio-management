...
def run_ensemble_strategy(df, unique_trade_date, rebalance_window, validation_window) -> None:
    ...
    ## training env
    train = data_split(df, start=20090000, end=unique_trade_date[0])
    ...
    for i in range(rebalance_window + validation_window, len(unique_trade_date), rebalance_window):
        ...
        last_state_ensemble = DRL_prediction(df=df, model=model_ensemble, name="ensemble_pickle",
        ...
        ############## Trading starts ##############
        print("======NLP Trading from: ", unique_trade_date[i - rebalance_window], "to ", unique_trade_date[i])
        #print("Used Model: ", model_ensemble)
        last_state_ensemble_nlp = DRL_prediction(df=df, model=model_ensemble, name="ensemble_pickle_nlp",
                                             last_state=last_state_ensemble_nlp, iter_num=i,
                                             unique_trade_date=unique_trade_date,
                                             rebalance_window=rebalance_window,
                                             turbulence_threshold=turbulence_threshold,
                                             initial=initial)
